...
namespace hhnl.HomeAssistantNet.Shared.Entities
{
    public abstract class OnOffEntity : BinaryEntity
    {
        ...
        {
            await HomeAssistantClient.CallServiceAsync(Domain, "turn_off", new { entity_id = UniqueId }, cancellationToken);
        }

        public async Task TurnOnAsync(CancellationToken cancellationToken = default)
        {
            await HomeAssistantClient.CallServiceAsync(Domain, "turn_on", new { entity_id = UniqueId }, cancellationToken);
        }

        public async Task ToggleAsync(CancellationToken cancellationToken = default)
        {
            await HomeAssistantClient.CallServiceAsync(Domain, "toggle", new { entity_id = UniqueId }, cancellationToken);
        }
